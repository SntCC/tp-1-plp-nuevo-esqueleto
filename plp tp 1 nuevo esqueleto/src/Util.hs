...
alinearDerecha n s = if n <= length s then s else replicate (n - length s) ' ' ++ s -- Faltaria agregar un If para cuando N es menor a la long de S
...
actualizarElem n f xs = zipWith (\i x -> if i == n then f x else x  ) [0 .. length (xs)-1] xs
